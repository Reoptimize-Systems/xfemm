...
void femmcli::LuaHeatflowCommands::registerCommands(LuaInstance &li)
{
    ...
    li.addFunction("hi_set_segment_prop", LuaCommonCommands::luaSetSegmentProperty);
    li.addFunction("hi_setsegmentprop", LuaCommonCommands::luaSetSegmentProperty);
...
/**
 * @brief Add the point (T,k) to the thermal conductivity vs. temperature curve for given material.
 * @param L
 * @return 0
 * \ingroup LuaHF
 *
 * \internal
 * ### Implements:
 * - \lua{hi_addtkpoint("materialname",T,k)}
 *
 * ### FEMM sources:
...
int femmcli::LuaHeatflowCommands::luaAddtkpoint(lua_State *L)
{
    ...
    m->Kn[m->npts++] = lua_todouble(L,2)+I*lua_todouble(L,3);
