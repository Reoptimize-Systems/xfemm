...
void CFemmeDoc::UnselectAll()
{
	...
	for(i=0;i < nodelist.size();i++) nodelist[i].IsSelected=FALSE;
	for(i=0;i < linelist.size();i++) linelist[i].IsSelected=FALSE;
	for(i=0;i < blocklist.size();i++) blocklist[i].IsSelected=FALSE;
	for(i=0;i < arclist.size();i++) arclist[i].IsSelected=FALSE;
}
...
BOOL CFemmeDoc::AddSegment(int n0, int n1, CSegment *parsegm, double tol)
{
	...
	CComplex p[2];
	CSegment segm;
	std::vector < CComplex, CComplex&> newnodes;
	...
	for(i=0,k=0;i<blocklist.size();i++)
		if(blocklist[i].BlockType!="<No Mesh>")
		{
			...
			if (blocklist[i].MagDirFctn.GetLength()>0)
				fprintf(fp,"	\"%s\"",blocklist[i].MagDirFctn);
			fprintf(fp,"\n");
...
void CFemmeDoc::UpdateUndo()
{
	...
	for(i=0;i<nodelist.size();i++) undonodelist.Add(nodelist[i]);
	for(i=0;i<linelist.size();i++) undolinelist.Add(linelist[i]);
	for(i=0;i<arclist.size();i++) undoarclist.Add(arclist[i]);
	for(i=0;i<blocklist.size();i++) undoblocklist.Add(blocklist[i]);
...
void CFemmeDoc::Undo()
{
	...
	std::vector < CNode, CNode&>             tempnodelist;
	std::vector < CSegment, CSegment&>       templinelist;
	std::vector < CArcSegment, CArcSegment&> temparclist;
	std::vector < CBlockLabel, CBlockLabel&> tempblocklist;
	...
	for(i=0;i<nodelist.size();i++) tempnodelist.Add(nodelist[i]);
	for(i=0;i<linelist.size();i++) templinelist.Add(linelist[i]);
	for(i=0;i<arclist.size();i++) temparclist.Add(arclist[i]);
	for(i=0;i<blocklist.size();i++) tempblocklist.Add(blocklist[i]);
	...
	for(i=0;i<undonodelist.size();i++) nodelist.Add(undonodelist[i]);
	for(i=0;i<undolinelist.size();i++) linelist.Add(undolinelist[i]);
	for(i=0;i<undoarclist.size();i++) arclist.Add(undoarclist[i]);
	for(i=0;i<undoblocklist.size();i++) blocklist.Add(undoblocklist[i]);
	...
	for(i=0;i<tempnodelist.size();i++) undonodelist.Add(tempnodelist[i]);
	for(i=0;i<templinelist.size();i++) undolinelist.Add(templinelist[i]);
	for(i=0;i<temparclist.size();i++) undoarclist.Add(temparclist[i]);
	for(i=0;i<tempblocklist.size();i++) undoblocklist.Add(tempblocklist[i]);
}
