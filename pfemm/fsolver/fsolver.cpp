...
int FSolver::LoadMesh()
{
	...
	for(i=0;i<k;i++){
		...
		if(elm.lbl<0){
			...
			fclose(fp);
			sprintf(infile,"%s.ele",PathName);	remove(infile);
			sprintf(infile,"%s.node",PathName);	remove(infile);
			sprintf(infile,"%s.pbc",PathName);	remove(infile);
			sprintf(infile,"%s.poly",PathName);	remove(infile);
			sprintf(infile,"%s.edge",PathName);	remove(infile);
			exit(1);
		}
