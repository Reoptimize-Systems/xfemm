...
static int str_find (lua_State *L)
{
            ...
            strpbrk(p, SPECIALS) == NULL)    /* or no special characters? */
    {
        ...
        if (s2)
        {
            lua_pushnumber(L, CComplex(s2-s+1,0));
            lua_pushnumber(L, CComplex(s2-s+l2,0));
            return 2;
        }
    }
    else
...
static int str_format (lua_State *L)
{
    ...
    while (*strfrmt)
    {
        ...
        else   /* format item */
        {
            ...
            case 'x':
            case 'X':
                sprintf(buff, form, (unsigned int) Re(luaL_check_number(L, arg)));
                break;
            case 'e':
            case 'E':
            case 'f':
